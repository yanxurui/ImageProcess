...
public class Common
{
     ...
     * @return
     */
    public static int[] grabber(Image im, int w, int h)
	{
		int [] pix = new int[w * h];
		try
		{
			//附加在 Image 或 ImageProducer 对象上获得图像像素子集的 ImageConsumer
     ...
     * @param str
     */
    public static Image createImage(int[] pixels,int iw,int ih)
    {
	 ...
	 * @param n
	 */
	public void chooseFile(JFileChooser jfc, int n)
    {
    	final int type = n;    
        //设置默认文件目录
        jfc.setCurrentDirectory(new File(getCurrentDirectory()));
	...
	//默认目录和文件
	private static String currentDirectory="images";
	private static String selectedFile="lenna.jpg";
	...
	{
		Common.selectedFile = selectedFile;
	}
...
// 		catch(InterruptedException e){ e.printStackTrace();}
	...
	//计算数组d最大值
    public static int maximum(int[] d)
    {
    	int m = 0;
    	for(int i = 0;i < 256; i++)
    	    if(d[i]> m) m = d[i];
